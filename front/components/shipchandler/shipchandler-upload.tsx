...
import { parseShipchandlerExcel } from "@/lib/excel-parser"
...
export function ShipchandlerUpload() {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && excelType) {
      ...
      try {
        if (excelType === "supply-order") {
          console.log('Procesando archivo Excel real:', file.name)
          
          // Leer archivo Excel real
          const realData = await parseShipchandlerExcel(file)
          console.log('Datos leídos del Excel:', realData)
          
          if (realData.length > 0) {
            // Convertir a formato esperado por la vista previa
            const formattedData = realData.map((item, index) => ({
              id: index + 1,
              fecha: item.fecha,
              clientes: item.clientes,
              desde: item.desde,
              subClientes: item.subClientes,
              hacia: item.hacia,
              bl: item.bl,
              buque: item.buque,
              tamano: item.tamano,
              numeroContenedor: item.numeroContenedor,
              ptgOrder: item.ptgOrder,
              status: item.status,
              voyage: item.voyage,
              tarifa: item.tarifa,
              gastosPuerto: item.gastosPuerto,
              otrosGastos: item.otrosGastos,
              jira: item.jira,
              fechaFacturacion: item.fechaFacturacion,
              driver: item.driver,
              plate: item.plate,
              bono: item.bono,
              rtContainer: item.rtContainer
            }))
            ...
            toast({
              title: "Sin datos",
              description: "No se encontraron datos válidos en el archivo Excel. Verifica que tenga las columnas correctas.",
              variant: "destructive",
            })
          }
        } else {
          // Para otros tipos, usar simulación temporal
          const dynamicData = simulateExcelParse("shipchandler", excelType)
          setPreviewData(dynamicData)
          toast({
            title: "Vista previa simulada",
            description: `Se han simulado ${dynamicData.length} registros para el tipo ${excelType}.`,
          })
        }
      } catch (error) {
        ...
        toast({
          title: "Error",
          description: `Error al procesar el archivo Excel: ${error.message}`,
          variant: "destructive",
        })
        setPreviewData([])
      } finally {
  ...
  const getTableHeadersForType = (type: string) => {
    const headers = {
      "supply-order": [
        "Fecha", "Cliente", "Desde", "Hacia", "B/L", "Buque", "Contenedor", "Tarifa", "Status", "Driver"
      ],
      "inventory": ["Producto", "Cantidad", "Precio", "Estado"],
      "delivery-note": ["Destinatario", "Producto", "Cantidad", "Estado"],
      "service-record": ["Servicio", "Cliente", "Fecha", "Estado"]
    }
    return headers[type] || []
  }
                    ...
                    <TableRow key={index}>
                      {excelType === "supply-order" && (
                        <>
                          ...
                          <TableCell>{row.clientes}</TableCell>
                          <TableCell>{row.bl}</TableCell>
                          <TableCell>{row.buque}</TableCell>
                          <TableCell>{row.tamano}"</TableCell>
                          <TableCell>{row.numeroContenedor}</TableCell>
                          <TableCell>${row.tarifa?.toFixed(2)}</TableCell>
                          <TableCell>
                            <Badge variant={row.status === "RT" ? "default" : "secondary"}>
                              {row.status}
                            </Badge>
                          </TableCell>
                        </>
...
const handleProcessData = () => {
  ...
  // Procesar los datos del Excel y guardarlos en el store
  ...
  // Agregar el archivo Excel al store
  dispatch(addExcelFile(excelFile))

  // Crear registros individuales para cada fila del Excel
  previewData.forEach((row, index) => {
    const record: ExcelRecord = {
      id: `${excelFile.id}-${index + 1}`,
      excelId: excelFile.id,
      module: "shipchandler",
      type: "supply-order",
      status: "pendiente",
      totalValue: row.tarifa + (row.otrosGastos || 0) + (row.bono || 0),
      data: row,
      createdAt: new Date().toISOString()
    }
    dispatch(addRecords([record]))
  })

  toast({
    title: "Datos procesados exitosamente",
    description: `Se han procesado ${previewData.length} registros de ${excelFile.filename}.`,
  })
...
// Actualizar el botón de procesar
<Button 
