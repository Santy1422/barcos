...
interface AgencyXmlViewerModalProps {
  open: boolean;
...
export function AgencyXmlViewerModal({ open, onOpenChange, invoice, onXmlSentToSap }: AgencyXmlViewerModalProps) {
  ...
  const handleDownloadXml = () => {
    ...
    const fileName = sapFileName || `AGENCY_INVOICE_${invoice.invoiceNumber}_${new Date().toISOString()}.xml`;
  ...
  const handleSendToSap = async () => {
    ...
    try {
      ...
      setLocalSapFileName(fileName);
      onXmlSentToSap?.();
      
      toast({ title: "XML enviado exitosamente", description: `Archivo ${fileName} enviado a SAP correctamente.` });
    } catch (error: any) {
          ...
          <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 p-4 rounded-lg">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div>
                ...
                <span className="ml-2">${(invoice.totalAmount || 0).toFixed(2)}</span>
              </div>
              {sapFileName && (
                <div>
