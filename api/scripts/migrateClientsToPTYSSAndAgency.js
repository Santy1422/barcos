...
const connectDB = async () => {
  try {
    // Usar la misma variable de entorno que usa el sistema
    const mongoUri = process.env.USER_MONGO_URI || process.env.MONGO_URI || 'mongodb://localhost:27017/barcos';
    ...
    console.log(`📊 URI: ${mongoUri.split('@')[1] || mongoUri.split('/').slice(-1)[0]}`);
  } catch (error) {
    console.error('❌ Error al conectar a MongoDB:', error);
    console.error('💡 Verifica que USER_MONGO_URI esté configurado en tu archivo .env');
    process.exit(1);
  }
};
...
const clientsSchema = new mongoose.Schema({
  ...
  createdAt: Date,
  updatedAt: Date
}, { timestamps: false, collection: 'clients' });
      ...
      try {
        ...
        if (Array.isArray(currentModule)) {
          ...
          await Clients.updateOne(
            ...
            { 
              $set: { 
                module: [...new Set([currentModule, 'ptyss', 'agency'])], // Evitar duplicados
                updatedAt: new Date()
              }
            }
          );
          const newModules = [...new Set([currentModule, 'ptyss', 'agency'])];
